  ...
  <form class="auth-form" @submit.prevent="submit">
    <div class="auth-grid-2">
      <div class="auth-field">
        <label class="auth-label" for="company-username">Username</label>
        <input
          id="company-username"
          ...
          type="text"
          placeholder="Company login username"
    ...
    <div class="auth-field">
      <label class="auth-label" for="company-industry">Industry</label>
      <input
        id="company-industry"
        ...
        type="text"
        placeholder="FinTech, SaaS, Manufacturing..."
        required
      />
    ...
    <div class="auth-grid-2">
      <div class="auth-field">
        <label class="auth-label" for="company-email">Company Email</label>
        <input
          id="company-email"
      ...
      <div class="auth-field">
        <label class="auth-label" for="company-password">Password</label>
    ...
    <div class="auth-grid-2">
      <div class="auth-field">
        <label class="auth-label" for="hr-name">HR Contact Name</label>
        <input
          id="hr-name"
          v-model.trim="form.hr_contact_name"
          class="auth-input"
          type="text"
      ...
      <div class="auth-field">
        <label class="auth-label" for="hr-email">HR Contact Email</label>
        <input
          id="hr-email"
          v-model.trim="form.hr_contact_email"
          class="auth-input"
          :class="{ 'input-error': errors.hr_contact_email }"
          type="email"
          placeholder="riya@company.com"
          required
        />
        <p v-if="errors.hr_contact_email" class="field-error">{{ errors.hr_contact_email }}</p>
      </div>
    </div>

    <div class="auth-field">
      <label class="auth-label" for="hr-phone">HR Contact Phone</label>
      <input
        id="hr-phone"
        v-model.trim="form.hr_contact_phone"
        class="auth-input"
        :class="{ 'input-error': errors.hr_contact_phone }"
        type="text"
        placeholder="10 digit mobile number"
        required
      />
      <p v-if="errors.hr_contact_phone" class="field-error">{{ errors.hr_contact_phone }}</p>
    </div>

    <button class="auth-submit" type="submit" :disabled="isLoading">
      {{ isLoading ? 'Submitting For Approval...' : 'Register Company (Needs Admin Approval)' }}
    </button>
  </form>
</template>
...
export default {
  ...
  methods: {
    validate() {
      ...
      const hrEmail = this.form.hr_contact_email.trim()
      if (!hrEmail || !hrEmail.includes('@') || !hrEmail.includes('.')) {
        errors.hr_contact_email = 'Enter a valid HR contact email.'
      }

      const phone = this.form.hr_contact_phone.trim()
      if (phone && !/^\d{10}$/.test(phone)) {
        errors.hr_contact_phone = 'Phone must be exactly 10 digits.'
      }
